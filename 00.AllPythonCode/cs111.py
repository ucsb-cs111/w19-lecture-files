#######################################################################
# These are .py versions of some of the codes I've used in class.
# You can access them from python or Jupyter by putting this file
# in the directory you are running from and saying:
#
#   import cs111
#
...
def draw_fern(nsteps = 1000, speed = 100):
    ...
    Alist = [ np.array( [[ .85,  .04], [-.04, .85]] ),               np.array( [[ .20, -.26], [ .23, .22]] ),               np.array( [[-.15,  .28], [ .26, .24]] ),               np.array( [[   0,    0], [   0, .16]] )]

    blist = [ np.array( [0, 1.6] ),               np.array( [0, 1.6] ),               np.array( [0, .44] ),               np.array( [0,   0] )]
